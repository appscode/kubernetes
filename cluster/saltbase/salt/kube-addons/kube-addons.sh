...
EOF
  ...
  local tries=100;
  local -r delay=10;
  # curl -v -H "Content-Type: application/json" -X POST -d "${engress}" http://localhost:8080/apis/appscode.com/v1/namespaces/appscode/ingresss && \
  while [ ${tries} -gt 0 ]; do
    [[ `curl -s -o /dev/null -w "%{http_code}" -H "Content-Type: application/json" -X POST -d "${engress}" http://localhost:8080/apis/appscode.com/v1/namespaces/appscode/ingresss` = '200' ]] && \
